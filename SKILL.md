...
Goal mode is not "write a longer first message". Treat it as task delegation: the user already knows the outcome they want, and Codex should break down the work, execute, verify, repair, and keep going until the acceptance criteria are met.

Coach beginners with this sentence when helpful:

> Plan 模式适合"想清楚"，Goal 模式适合"做到底"。先 Plan 再 Goal，是新手最稳的用法。

A good Goal usually contains five practical ingredients:

1. **最终目标** - 完成什么，不要只说"优化一下"。
2. **验收标准** - 做到什么算完成，最好 3-7 条。
3. **约束条件** - 不要大改架构、不要破坏现有功能、保持现有风格。
4. **验证方式** - 运行测试、启动页面、检查 diff、浏览器验证等。
5. **反馈机制** - 关键选择先问；报错先分析再修；阶段完成后简短汇报。
...
Constraints:
  ...
  - <collaboration rules: ask before key choices, show full errors before fixing, summarize phase progress>
...
### Step -1: Decide whether Goal is the right tool

Before collecting Goal fields, do a quick readiness check. If the user already knows the desired result and wants Codex to execute until it is verifiable, continue with this skill. If the user is still exploring, recommend Plan first.

Good fit for Goal mode:

- A clear small feature
- A reproducible bug fix
- A bounded refactor
- Adding tests to known code
- A page / interaction polish pass
- Converting a batch of material into a fixed format
- Any task that should keep going until it reaches an acceptance checklist

Poor fit for immediate Goal mode:

- The user does not yet know what they want
- The project structure has not been inspected
- The request is too large, e.g. "build a complete product"
- The user is mainly learning, discussing, or comparing approaches

If it is a poor fit, do **not** force a `/goal`. Say, in Chinese when the user is Chinese-speaking:

> 这件事建议先 Plan，再 Goal：先让 Codex 阅读项目并给方案，不改代码；方案确认后，再把它转成可执行的 Goal。

You may offer this starter prompt, but do not generate a full `/plan` template unless the user asks:

```
请先进入 Plan 模式，阅读项目并给我方案，不要改代码。
```
...
- Include collaboration rules when useful: key choices require a recommended option + reason before proceeding; errors require the full original error before analysis and repair; each phase should end with a brief progress summary
...
- **Goal readiness**: if the user is still exploring or the scope is unknown, recommend Plan first instead of rendering
...
3. **Never use Goal as discovery when Plan is needed** — if the user is still deciding what to build, recommend Plan first, then turn the approved plan into a Goal.
...
6. **For SDD-driven goals (scenario B), the first action is always "read X files and report counts"** — bypasses `@filename` reference uncertainty and exposes loading failures early.
...
- **User still thinking through the task**: do not render a Goal yet. Recommend Plan first, then convert the approved plan into a Goal.
...
- Does not generate full prompts for `/plan`, `/compact`, or other Codex commands; it may recommend Plan first when Goal is premature
